...
public class WeekView extends View {
    ...
    private void cacheEvent(WeekViewEvent event) {
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            WeekViewEvent event1 = new WeekViewEvent(event.getId(), event.getName(), event.getLocation(), event.getDescription(), event.getStartTime(), endTime);
            ...
            WeekViewEvent event2 = new WeekViewEvent(event.getId(), event.getName(), event.getLocation(), event.getDescription(), startTime, event.getEndTime());
    ...
    public void computeScroll() {
        ...
        if (!mScroller.isFinished() && (mCurrentFlingDirection != Direction.NONE) && (mScroller.getCurrVelocity() <= ViewConfiguration.get(getContext()).getScaledMinimumFlingVelocity())) {
            goToNearestOrigin();
